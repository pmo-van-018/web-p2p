<template>
  <order-grid-renderer class="management-orders">
    <template #head>
      <pending-filter-head @on-filters="$pendingOrdersMixin_onFilterOrder" />
    </template>
    <template #body>
      <list-common
...
export default {
  ...
  mixins: [walletMixin, userBuyOrderMixin, modalMixin, pendingOrdersMixin],
  async created () {
    await this.$pendingOrdersMixin_onResetFilter()
  },
  methods: {
    onOpenDetail (order) {
