  ...
  <section class="pending-order">
    <div class="pending-order__container">
      <div class="pending-order__head">
        ...
        />
        <div class="pending-order__head--receive-limit">
          {{ $t('merchant.list_order.order_receive_limit') }}: <strong>{{ totalPicked }}/{{ appealReceivedBySupporterLimit }}</strong>
        </div>
      </div>
...
import { GET_SETTING_LIMIT_BY_SUPPORTER } from '@/api/master-data';
...
export default {
  ...
  mixins: [supporterMixin, commonOrdersMixin],
  props: {
    orders: {
      type: Array,
      default: () => []
  ...
  data () {
    return {
      appealReceivedBySupporterLimit: 0,
      totalPicked: 0,
      fields: [
        {
          key: 'type',
  ...
  created () {
    this.getReceiveLimit()
    this.countPickedOrder()
  },
  methods: {
    async getReceiveLimit () {
      try {
        const setting = await this.$axios.$get(GET_SETTING_LIMIT_BY_SUPPORTER)
        this.appealReceivedBySupporterLimit = setting?.data?.appealReceivedBySupporterLimit || 0
      } catch (e) {
        console.error(e)
      }
    }
