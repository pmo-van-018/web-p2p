      ...
      <div class="order-management__body">
        ...
        >
            ...
            <div class="description-text">
              <div
                :class="[
                  ...
                  { 'description-text__title--grey': isTimeOut(item) }
                ]"
              >
          ...
          <template #cell(actions)="{ item }">
            <div class="description-text text-right" @click="$_appealSupporterMixin_onOpenAppealDetail(item.id)">
              <div
                class="description-text__title description-text__title--underline description-text__title--green font-weight-bold"
              >
                {{ $t('action.view_more') }}
              </div>
            </div>
...
import { STATUS_HISTORY_ORDER } from '@/resources/order-filters'
...
export default {
  ...
  computed: {
    queryParams () {
      return {
        page: this.page,
        limit: this.limit,
        supporterId: this.user.id,
        ...((this.searchField && this.searchValue) && { searchField: this.searchField }),
        ...(this.searchValue && { searchValue: this.searchValue }),
        orderStatus: this.orderStatus ? this.orderStatus : STATUS_HISTORY_ORDER,
  ...
  async created () {
    this.tabActiveCallApi = 'completed'
    await Promise.all([
      this.countOpeningAppeal(),
      this.$_appealSupporterMixin_onSortAppeal({
        sortField: 'updatedAt',
        sortDirection: 'DESC'
      })
    ])
