...
import { LIST_STAFFS_BY_MANAGER } from '@/api/staff';
...
export default {
  ...
  methods: {
    ...
    async $_staffMixin_onParamsChange ({
      page,
      limit
    }) {
      this.page = page;
    ...
    async $_staffMixin_getStaffs (isMergeOldData = false) {
      try {
        ...
        const res = await this.$axios.$get(LIST_STAFFS_BY_MANAGER(this.$route.params.id), { params });
        if (!isEmpty(res?.data)) {
